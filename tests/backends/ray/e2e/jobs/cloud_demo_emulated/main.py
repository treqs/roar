"""Cloud-demo-shaped pipeline entrypoint for fragment sufficiency contracts."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="cloud-demo-emulated")
    parser.add_argument("--run-id", default=None)
    args = parser.parse_args()

    run_id = args.run_id or f"run-{uuid.uuid4().hex[:8]}"
    state_file = Path(f"/tmp/cloud-demo-emulated-state-{run_id}.json")
    ...
    print(
        json.dumps(
            {
                "script": "cloud_demo_emulated",
                "run_id": run_id,
                "shard_keys": final_state.get("shard_keys", []),
                "model_key": final_state.get("model_key"),
                "metrics_key": final_state.get("metrics_key"),
