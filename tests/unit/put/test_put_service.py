...
class TestPutServiceBasic:
    ...
    def test_prepare_artifacts_for_registration_normalizes_source_type_and_hash_algorithms(
        self, tmp_path: Path
    ):
        """Prepared registration payload keeps hash algorithms and sanitizes source_type."""
        backend = MemoryBackend(bucket="bucket", prefix="")
        mock_db = Mock()
        glaas_deps = create_mock_glaas_deps()
        service = PutService(
        ...
        artifacts = [
            {
                ...
                "kind": "composite",
                "hashes": [
                    {"algorithm": "composite-blake3", "digest": "A" * 64},
            ...
            },
            {
                "size": 5,
                "source_type": "GS",
                "hashes": [{"algorithm": "sha256", "digest": "C" * 64}],
            },
        ]

        prepared = service._prepare_artifacts_for_registration(artifacts, "session-hash")

        # The composite artifact (composite-blake3) is filtered out; only the
        # non-composite artifact should be prepared for the batch endpoint.
        assert len(prepared) == 1
        assert prepared[0]["source_type"] == "gs"
        assert prepared[0]["hashes"] == [{"algorithm": "sha256", "digest": ("c" * 64)}]

    def test_prepare_artifacts_for_registration_filters_out_composite_artifacts(
        self, tmp_path: Path
    ):
        """Composite artifacts (composite-blake3) are filtered out of batch registration."""
        backend = MemoryBackend(bucket="bucket", prefix="")
        mock_db = Mock()
        glaas_deps = create_mock_glaas_deps()
        service = PutService(
        ...
        artifacts = [
            ...
            {
                "kind": "primitive",
                "hashes": [{"algorithm": "blake3", "digest": "e" * 64}],
                "size": 10,
                "source_type": "ftp",
            },
        ]
        ...
        # The composite artifact is filtered out; only the primitive remains.
        assert len(prepared) == 1
        assert prepared[0]["hashes"] == [{"algorithm": "blake3", "digest": ("e" * 64)}]
        assert prepared[0]["source_type"] is None
