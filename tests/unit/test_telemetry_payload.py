...
def test_payload_import_does_not_pull_forbidden_modules() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    script = """
import json
import sys

import roar.telemetry.payload

names = [
    "roar.execution.provenance",
    "roar.application.publish",
    "roar.db",
    "roar.integrations.glaas.client",
    "roar.integrations.telemetry",
    "urllib.request",
]
...
"""
    ...
    assert loaded == {
        "roar.execution.provenance": False,
        "roar.application.publish": False,
        "roar.db": False,
        "roar.integrations.glaas.client": False,
        "roar.integrations.telemetry": False,
        "urllib.request": False,
    }


def test_only_uploader_module_imports_urllib() -> None:
    telemetry_dir = Path(__file__).resolve().parents[2] / "roar" / "telemetry"

    offenders = [
        path.name
        for path in telemetry_dir.glob("*.py")
        if path.name != "uploader.py" and "urllib" in path.read_text(encoding="utf-8")
    ]
