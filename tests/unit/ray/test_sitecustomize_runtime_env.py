...
def test_patch_ray_init_skips_pip_dependency_by_default(
    ...
    assert {"USER_KEY", "ROAR_WORKER", "ROAR_JOB_ID", "ROAR_DRIVER_JOB_UID"}.issubset(
    ...
    class _FakeCollectorActor:
        ...
        def options(cls, **kwargs):
            ...
            return SimpleNamespace(
                remote=lambda: SimpleNamespace(ping=cls._FakeRemoteMethod()),
            )
    ...
    class _FakeCollectorActor:
        ...
        def options(cls, **kwargs):
            ...
            def _remote(**actor_kwargs):
                ...
                return SimpleNamespace(ping=cls._FakeRemoteMethod())
...
def test_prepare_worker_runtime_env_sets_roar_worker_and_preload(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
...
def test_prepare_worker_runtime_env_does_not_write_worker_sitecustomize(
    ...
    working_dir = Path(str(prepared["working_dir"]))
    worker_sitecustomize = working_dir / "sitecustomize.py"
    assert not worker_sitecustomize.exists()
