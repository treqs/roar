...
from roar.ray.fragment_key import (
    generate_fragment_key,
    key_file_path,
    load_key,
    save_key,
)
...
def test_store_key_writes_json_file_to_roar_fragment_sessions_dir(tmp_path: Path) -> None:
    ...
    path = save_key(tmp_path / ".roar", key)

    assert path == key_file_path(tmp_path / ".roar", key["session_id"])
...
def test_load_key_round_trip_restores_session_id_and_token(tmp_path: Path) -> None:
    ...
    save_key(roar_dir, key)
    restored = load_key(roar_dir, key["session_id"])
...
def test_missing_key_raises_file_not_found(tmp_path: Path) -> None:
    with pytest.raises(FileNotFoundError):
        load_key(tmp_path / ".roar", "00000000-0000-4000-8000-000000000000")
