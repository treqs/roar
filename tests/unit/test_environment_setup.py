...
# -- Python interpreter pinning (reproduce "same setup") --
...
class TestPythonPinning:
    def _svc(self):
        svc = EnvironmentSetupService()
        svc._logger = MagicMock()
        svc._presenter = MagicMock()
        return svc

    def test_candidates_exact_then_minor(self):
        svc = self._svc()
        assert svc._python_candidates("3.14.4") == ["3.14.4", "3.14"]
        assert svc._python_candidates("3.14") == ["3.14"]
        assert svc._python_candidates(None) == []

    def test_recorded_version_from_pipeline_runtime(self):
        svc = self._svc()
        pipeline = MagicMock()
        pipeline.build_steps = []
        pipeline.run_steps = [
            {"metadata": json.dumps({"runtime": {"python": {"version": "3.14.4"}}})}
        ]
        assert svc._recorded_python_version(pipeline) == "3.14.4"

    def test_recorded_version_none_when_absent(self):
        svc = self._svc()
        pipeline = MagicMock()
        pipeline.build_steps = []
    ...
    def test_uv_venv_pins_recorded_version(self, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            ...
            (venv_dir / "pyvenv.cfg").write_text("version = 3.14.4\n")
            return MagicMock(returncode=0)

        with patch("subprocess.run", side_effect=fake_run) as run:
            svc._create_venv(repo_dir, "3.14.4")
        # First (and only) uv call pins the exact recorded version.
    ...
    def test_uv_falls_back_to_minor_then_default_with_warning(self, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            calls.append(cmd)
            # exact (3.14.4) and minor (3.14) both fail; bare default succeeds
            if "--python" in cmd:
                return MagicMock(returncode=1, stderr="no such python")
            venv_dir.mkdir(parents=True, exist_ok=True)
            (venv_dir / "pyvenv.cfg").write_text("version = 3.13.14\n")
            return MagicMock(returncode=0)
