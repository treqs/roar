...
''')
    ...
    result = roar_cli("run", "python", "noop_write.py", check=False)
...
''')
    ...
    # Run the script with roar
    result = roar_cli("run", "python", "real_write.py", check=False)
    assert result.returncode == 0, (
        f"roar run failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
...
''')
    ...
    # Run the script with roar
    result = roar_cli("run", "python", "truncate_only.py", check=False)
    assert result.returncode == 0, (
        f"roar run failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
    ...
    try:
        ...
        assert len(truncate_outputs) == 0, (
            f"truncate_me.txt was incorrectly recorded as output. "
            f"Output paths: {output_paths}"
        )
    finally:
        conn.close()
