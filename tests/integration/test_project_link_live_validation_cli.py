...
class _FakeTreqsServer(ThreadingHTTPServer):
    last_authorization: str | None = None


class _FakeTreqsHandler(BaseHTTPRequestHandler):
    server: _FakeTreqsServer

    def do_GET(self) -> None:
        if self.path != "/api/v1/user/access-context":
...
@pytest.fixture
def fake_treqs_server() -> _FakeTreqsServer:
    server = _FakeTreqsServer(("127.0.0.1", 0), _FakeTreqsHandler)
...
def test_project_link_validates_against_access_context_server(
    ...
    assert "Validated binding against treqs access context" in result.stdout
    assert fake_treqs_server.last_authorization == "Bearer access-token"
...
def test_project_link_rejects_unknown_project_from_access_context(
    ...
    combined_output = f"{result.stdout}\n{result.stderr}"
    assert result.returncode != 0
    assert "Project not available in treqs access context: proj-missing" in combined_output
