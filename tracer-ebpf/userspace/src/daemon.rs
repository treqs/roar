...
impl DaemonState {
    ...
    pub fn deregister(&mut self, run_id: u64) -> usize {
        if let Some(run) = self.runs.get_mut(&run_id) {
            ...
            run.tracer.end_time = SystemTime::now()
                .duration_since(SystemTime::UNIX_EPOCH)
                .unwrap_or_default()
                .as_secs_f64();
        ...
        if tag == roar_ebpf_common::TAG_LARGE && data.len() >= 4 + 8 {
            ...
            if event_type == roar_ebpf_common::EventType::Clone as u16 {
                ...
                if data.len() >= 28 {
                    ...
                    let child_pid =
                        u64::from_ne_bytes(child_pid_bytes.try_into().unwrap()) as u32;
                    if child_pid > 0 {
...
/// Load BPF programs and attach all tracepoints. Returns the Ebpf handle.
fn load_and_attach_bpf() -> Result<Ebpf> {
...
pub fn run_daemon(socket_path: PathBuf, idle_timeout: Duration) -> Result<()> {
    ...
    let mut bpf = load_and_attach_bpf()?;
            ...
            ClientMessage::GetReport { run_id } => {
                ...
                match report {
                    Some(data) => {
                        ipc::send_message(
                            &mut stream,
                            &DaemonMessage::Report { run_id, data },
                        )?;
                    }
                    None => {
                        ipc::send_message(
...
mod tests {
    ...
    fn test_get_report_returns_real_data() {
        ...
        state.runs.get_mut(&1).unwrap().tracer.handle_open(
            100,
            3,
    ...
    fn test_process_event_routes_to_correct_run() {
        ...
        let fd1 = state.runs[&1].tracer.fd_state.get(&(100, 3)).unwrap();
        assert!(fd1.was_written);

        let fd2 = state.runs[&2].tracer.fd_state.get(&(200, 3)).unwrap();
