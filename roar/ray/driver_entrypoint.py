...
def _entry_to_ref(entry: S3LogEntry) -> tuple[str, ArtifactRef] | None:
...
def _emit_driver_proxy_fragment(fragment: TaskFragment) -> None:
    ...
    glaas_url = str(os.environ.get("GLAAS_URL") or os.environ.get("GLAAS_API_URL") or "").strip()
    ...
    try:
        try:
            ...
            if handle is not None:
                child_env["ROAR_DRIVER_PROXY_PORT"] = str(handle.port)
