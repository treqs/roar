...
from ..integrations import register_telemetry_provider, register_vcs_provider
...
def _discover_entrypoint_plugins() -> None:
    ...
    try:
        ...
        for ep in eps:
            try:
                plugin_cls = ep.load()
                _register_entrypoint_plugin(plugin_cls)
...
def _register_entrypoint_plugin(plugin_cls: type) -> None:
    """Register an entry point plugin based on its interface."""
    if _implements(plugin_cls, ITelemetryProvider):
        instance = plugin_cls()
        register_telemetry_provider(instance.name, plugin_cls)
    elif _implements(plugin_cls, IVCSProvider):
        instance = plugin_cls()
        register_vcs_provider(instance.name, plugin_cls)
